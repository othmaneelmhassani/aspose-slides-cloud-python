...
class TestLayoutSlides(BaseTest):
    def setUp(self):
    ...
    def test_layout_slide_delete_unused(self):
        BaseTest.slides_api.copy_file("TempTests/" + constant.FILE_NAME, constant.FOLDER_NAME + "/" + constant.FILE_NAME)
        layout_slides_before = BaseTest.slides_api.get_layout_slides(constant.FILE_NAME, constant.PASSWORD, constant.FOLDER_NAME)
        self.assertEqual(11, len(layout_slides_before.slide_list))

        layout_slides_after = BaseTest.slides_api.delete_unused_layout_slides(constant.FILE_NAME, constant.PASSWORD, constant.FOLDER_NAME)
        self.assertEqual(2, len(layout_slides_after.slide_list))

    def test_layout_slide_delete_unused_online(self):
        with open(constant.LOCAL_TEST_DATA_FOLDER + "/" + constant.FILE_NAME, 'rb') as f:
            file = f.read()

        result = BaseTest.slides_api.delete_unused_layout_slides_online(file, constant.PASSWORD)
